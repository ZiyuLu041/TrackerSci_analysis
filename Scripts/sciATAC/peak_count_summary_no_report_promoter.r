...
cell_nread_file = paste(peak_count_folder, "cell_nread.txt", sep = "")
cell_nread = fread(cell_nread_file)
colnames(cell_nread) = c("id", "total_reads",'mapped_reads','mapped_ratio')
rownames(cell_nread) = cell_nread$id
cell_annot$total_reads = cell_nread[peak_cell$id,'total_reads']
...
peak_bed_file = paste(merged_peak_folder, "promoter_gtf.overlappingmerged.bed", sep = "")
peak_bed = fread(peak_bed_file)
colnames(peak_bed) = c("chr", "start", "end", "gene")
...
save(df_cell, df_peak, peak_count, file = output_file)
