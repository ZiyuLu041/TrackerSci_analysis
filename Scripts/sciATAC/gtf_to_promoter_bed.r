...
gtf_pos["end"] = gtf_pos$V4 + downstream
...
gtf_neg["start"] = gtf_neg$V5 - downstream
gtf_neg["end"] = gtf_neg$V5 + upstream

gtf_neg["start"][gtf_neg["start"] < 0] = 0

# combine the start and end site
gtf_combine = rbind(gtf_pos, gtf_neg) %>% select(c(V1, start, end, gene))
gtf_combine$start <- str_trim(format(gtf_combine$start, scientific = FALSE), side="both")
gtf_combine$end <- str_trim(format(gtf_combine$end, scientific = FALSE), side="both")
