...
def peak_count(sample, sam_folder, DHS, sample_ID, dis,output_folder):
    ...
    total_read_output = output_folder + "/" + sample + ".nread.txt"
    
    peak_ratio = float(mapped_reads)/float(total_reads)
    with open(total_read_output, 'w') as total_read_output:
        line = str(cell_ID) + "\t" + str(total_reads) + '\t' + str(mapped_reads) +  '\t' + str(round(peak_ratio, 9)) + '\n'
        total_read_output.write(line)
    return 0
...
func = partial(peak_count, sam_folder = sam_folder, DHS = DHS, sample_ID = sample_ID, dis = dis,output_folder=output_folder)
